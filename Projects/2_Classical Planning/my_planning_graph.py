...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        """ Return True if the effects of either action 
            negate the preconditions of the other 
    ...
    def _competing_needs(self, actionA, actionB):
        """ Return True if any preconditions of the two actions 
            are pairwise mutex in the parent layer
...
class PlanningGraph:
    ...
    def goals_are_mutex(self, layer):
        for goal_a in self.goals:
            for goal_b in self.goals:
    ...
    def h_levelsum(self):
        ...
        goal_levels = {goal:-1 for goal in self.goals}
        level_sum, goals_met, i = 0, 0, 0
        ...
        while not self._is_leveled:
            ...
            for goal in self.goals:
                if goal in layer:
                    if goal_levels[goal] == -1:
                        goal_levels[goal] = i
                        goals_met = goals_met + 1                        
            if goals_met == num_goals:
                break           
            self._extend() 
            i = i + 1   
            
        for goal in goal_levels:
            level_sum = level_sum + goal_levels[goal]
    ...
    def h_setlevel(self):
        ...
        # for i, layer in enumerate(self.literal_layers):
        #    if self.all_goals_met(layer):
        #        if not self.goals_are_mutex(layer):
        #            return i
        # return len(self.literal_layers)
